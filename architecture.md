...
> ⚠️ **Min-length filter:** The loader applies a minimum review length of **2 characters** (not 20) — very short entries are dropped but the pipeline relies on the relevance filter to further screen low-quality reviews.
...
filtered_df (n reviews)
     │
     ├─ chunk into batches of 12
     │
     ├── CALL 1: Theme Extraction (max 3 batches → up to 36 reviews sent to LLM)
...
- **Review-count slider** *(visible only in Live Collection mode)*:
  ...
  - Split: **~60% Play Store / ~20% App Store / ~20% Spotify Community**
...
- [x] Spotify dark theme applied throughout
...
| Country | `in` |
| Sort | Newest |
| Count | `int(scrape_count × 0.60)` — slider-driven, min 10 |

Returns: `[{source, review_text, date, rating, language}]`
...
| Region | `in` |
| Sort | Most Recent |
| Count | `int(scrape_count × 0.20)` — slider-driven, min 10 |

> **Note:** Apple iTunes RSS caps at 100 reviews per request. Title is prepended to body text for fuller context.

Returns: `[{source, review_text, date, rating, language}]`

---

### Sub-Phase 5c — Spotify Community Forum Scraper (`modules/scraper_spotify_community.py`)

| Config | Value |
|--------|-------|
| Method | Lithium/Khoros RSS feeds — no auth required |
| Boards | Music (`/t5/Music/rss`), Features (`/t5/Content-Questions/rss`), Ideas (`/t5/Live-Ideas/rss`) |
| Keyword Filter | Discovery-relevant terms only (post-fetch filter) |
| Count | `scrape_count - play_count - app_count` — slider-driven, min 10 |

Returns: `[{source: "Spotify Community", review_text, date, rating: None, language: "en"}]`

---

### Sub-Phase 5d — Reddit Scraper (`modules/scraper_reddit.py`) *(Not active in Live flow)*

Fully implemented using OAuth2 app-only flow targeting `r/spotify`, `r/SpotifyThefts`, and `r/ifyoulikeblank`. **Not wired into the Live Collection active flow** — available for future activation.

| Config | Value |
|--------|-------|
| Auth | OAuth2 client credentials (no user login) |
| Env vars required | `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET` |
| Subreddits | `r/spotify`, `r/SpotifyThefts`, `r/ifyoulikeblank`, `r/ListenToThis` |
| Fallback | Returns `[]` silently if credentials are missing |

---

### Sub-Phase 5e — Sidebar Mode Toggle

```
Mode = "Preloaded Dataset"  → load_preloaded_reviews()
Mode = "Live Collection"    → scrape_playstore_reviews() [60%]
                              + scrape_appstore_reviews() [20%]
                              + scrape_spotify_community() [20%]
                              → combine all_scraped → pd.DataFrame
                              → same filter + AI pipeline

Fallback: if all scrapers return [] → load_preloaded_reviews() + st.warning
```

All scraper outputs are combined, converted to DataFrame matching the loader schema, then passed through the same `filter_relevant()` → `run_analysis()` pipeline.

### Validation Checkpoint ✅
- [x] Live mode fetches > 0 reviews from at least one source
- [x] Scraper failures are caught and surfaced as `st.warning`, not crashes
- [x] Analysis pipeline works identically for both modes
- [x] Status toast shows per-source counts (e.g. "250 Play Store · 80 App Store · 45 Community")
...
### UI Placement
- Below all 7 tabs (only visible when report exists)
...
### Validation Checkpoint ✅
- [x] Downloaded Markdown is valid and readable
- [x] Downloaded CSV opens correctly in Excel/Sheets
- [x] Buttons only appear when a report exists in session state
...
app.py
  ...
  └── modules/scraper_reddit.py  ← implemented, not active in Live Collection flow
```
